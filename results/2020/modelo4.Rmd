...
Computed from 2500 by 380 log-likelihood matrix
         ...
         Estimate   SE
elpd_loo  -1062.2 17.8
p_loo        22.7  1.3
looic      2124.4 35.7
------
Monte Carlo SE of elpd_loo is 0.5.s
...
```{r}
times <- dados |> select(home_team_name) |> distinct() |> arrange() 
...
fit_summary_att <- summary(fit_sim_modelo4, pars = eff_aleatorios_att)$summary
fit_summary_att <- fit_summary_att[, c("50%", "2.5%", "97.5%")]

fit_summary_def <- summary(fit_sim_modelo4, pars = eff_aleatorios_def)$summary
