...
```{r}
modelo2 <- stan_model(file = "../../models/stan/binomial-negativa-misto.stan")
```
...
Ajuste do modelo
```{r}
fit_sim_modelo2 <- sampling(modelo2, data = dados_list, chains = 1, iter = iter, cores = 1, refresh = 100)
```

```{r}
list_of_results <- extract(fit_sim_modelo2, pars = c("y1_tilde", "y2_tilde"))
...
ggplot(data = predict_data, mapping = aes(x = round)) + 
  ...
  geom_line(aes(y= median, colour="Modelo Binomial Negativa Misto")) +
  scale_color_manual(name = "", values = c("Observado" = "darkblue", "Modelo Binomial Negativa Misto" = "red")) +
...
EQM e Vies de gols
```{r}
list_of_goals <- rstan::extract(fit_sim_modelo2, pars = c("y1_tilde", "y2_tilde"))
...
```{r}
predict_data |> 
  filter(team == "Atletico-MG" & round >= 33) |>
  arrange(round) |>
  rename(
    Rodada = round,
    Time = team,
    "Pontuação observada" = team_points,
    Mediana = median,
    "2,5%" = lower_bound,
    "97,5%" = upper_bound
  )
...
```{r}
list_of_fixed_parameters <- rstan::extract(fit_sim_modelo2, pars = c("home", "mu_att", "mu_def", "sigma_att", "sigma_def"))
```

```{r}
MCMCsummary(fit_sim_modelo2) |>
  select(
    Rhat,
    n.eff
  ) |>
